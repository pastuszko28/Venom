...
const outComponentsPath = path.join(outDir, "optional-module-components.generated.server.ts");
...
function buildEntries() {
  ...
  for (const manifestPath of collectManifestPaths()) {
    ...
    const moduleDirName = path.basename(manifestDir);
    const pageCandidates = [
      path.join(manifestDir, "web-next", "page.tsx"),
      path.join(manifestDir, "web-next", "page.ts"),
      path.join(manifestDir, "web-next", "page.jsx"),
      path.join(manifestDir, "web-next", "page.js"),
    ];
    const existingPagePath = pageCandidates.find((candidate) => existsSync(candidate)) ?? null;
...
function renderComponentLoadersFile(entries) {
  const loaderLines = entries
    .filter((entry) => entry.existingPagePath)
    .map((entry) => {
      const importPath = `@/optional-modules/${entry.moduleDirName}/page`;
      return `  "${entry.moduleId}": async () => (await import("${importPath}")).default ?? null,`;
    })
    .join("\n");
...
function main() {
  ...
  writeFileSync(outComponentsPath, renderComponentLoadersFile(entries));
  console.log(`[modules] Wrote ${outPath} -> ${entries.length} module(s)`);
  console.log(
    `[modules] Wrote ${outComponentsPath} -> ${entries.filter((entry) => entry.existingPagePath).length} loader(s)`,
  );
}
